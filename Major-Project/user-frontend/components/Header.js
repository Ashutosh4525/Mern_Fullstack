...
export default function Header(){
    ...
    const handleScroll = () => {
      ...
      if (currentY > lastScroll.current && currentY > 20) {
        setShowHeader(false)
      } else {
        setShowHeader(true)
      }
    ...
    return(
        <>
        <header className={`fixed inset-x-0 top-0 z-50 ${showHeader ? "translate-y-0" : "-translate-y-full opacity-0"}`}>
        ...
        <Dialog open={mobileMenuOpen} onClose={setMobileMenuOpen} className="lg:hidden">
          ...
          <DialogPanel className="fixed inset-y-0 right-0 z-50 w-full max-w-sm overflow-y-auto bg-black/95 backdrop-blur-2xl p-6 border-l border-white/10">
            ...
            <div className="flex items-center justify-between">
              ...
              <AuthActions onAction={() => setMobileMenuOpen(false)}/>
            </div>
          </DialogPanel>
        </Dialog>
      </header>
